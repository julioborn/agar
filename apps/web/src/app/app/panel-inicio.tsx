...
import {
  ...
  Beef, Fence, Rows3, Tag, BarChart3, ChevronDown, TrendingUp, Building2,
} from 'lucide-react';

type Seccion = 'agricultura' | 'ganaderia';
...
interface Props {
  ...
  rolLabel: string;
  fechaLabel: string;
  children?: React.ReactNode; // KPI cards, renderizadas por el servidor
}

const COLOR: Record<Seccion, { solido: string; sombra: string }> = {
  agricultura: { solido: '#006836', sombra: 'shadow-[#006836]/20' },
  ganaderia:   { solido: '#a83a3a', sombra: 'shadow-[#a83a3a]/20' },
};

export default function PanelInicio({ esLector, empresaNombre, rolLabel, fechaLabel, children }: Props) {
  const [seccion, setSeccion] = useState<Seccion>('agricultura');
  ...
  const links = seccion === 'agricultura' ? agricultura : ganaderia;
  const color = COLOR[seccion];

  return (
    <div className="space-y-6">

      {/* ── Encabezado: color en sintonía con la sección elegida ────────── */}
      <div
        className={cn('rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden shadow-md transition-colors duration-300', color.sombra)}
        style={{ backgroundColor: color.solido }}
      >
        <div className="relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight leading-tight text-white">{empresaNombre}</h1>
            </div>
          </div>
        </div>
      </div>

      {/* ── KPI Cards (renderizadas por el servidor) ────────────────────── */}
      {children}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => setSeccion('agricultura')}
            className={cn(
          ...
          >
            ...
            <ChevronDown className={cn('w-4 h-4 shrink-0 transition-transform duration-200', seccion === 'agricultura' && 'text-white')} />
          </button>

          <button
            type="button"
            onClick={() => setSeccion('ganaderia')}
            className={cn(
              'flex items-center gap-3 sm:gap-4 rounded-2xl border p-4 sm:p-5 transition-all duration-200 text-left',
              seccion === 'ganaderia'
          ...
          >
            ...
            <ChevronDown className={cn('w-4 h-4 shrink-0 transition-transform duration-200', seccion === 'ganaderia' && 'text-white')} />
          </button>
        </div>
              ...
              <div className={cn(
                ...
                link.primary
                  ...
                  : cn(
                      ...
                      seccion === 'agricultura' ? 'hover:border-[#006836]/30' : 'hover:border-[#a83a3a]/30',
                    ),
              )}>
                <link.icon className={cn('w-5 h-5', link.primary ? 'text-white' : seccion === 'agricultura' ? 'text-[#006836]' : 'text-[#a83a3a]')} />
                <div className="mt-6">
