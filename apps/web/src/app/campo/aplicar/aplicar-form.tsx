  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-6 text-center">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900">¡Aplicación registrada!</h2>
          <p className="text-slate-500 text-sm mt-1">El stock se actualizó correctamente.</p>
        </div>
        <div className="flex flex-col gap-3 w-full max-w-xs">
          ...
          <button
            ...
            className="w-full bg-white border border-slate-200 text-slate-700 font-semibold py-3.5 rounded-xl text-base transition-colors"
          >
            Volver al inicio
          </button>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1.5">Cultivo *</label>
          <select
            value={cultivoId}
            onChange={(e) => setCultivoId(e.target.value)}
            className="w-full rounded-xl border border-slate-300 px-4 py-3 text-base text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-700">Productos aplicados</h2>
          <button
            type="button"
            onClick={addItem}
            className="flex items-center gap-1.5 text-green-700 text-sm font-semibold py-1.5 px-3 rounded-lg bg-green-50 hover:bg-green-100 active:bg-green-200 transition-colors"
          >
        ...
        {items.map((item, idx) => {
          ...
          const perdida   = retirada > 0
            ? Math.max(0, retirada - (aplicada || retirada) - devuelta)
            : 0;

          return (
            <div key={item.key} className={`bg-white rounded-2xl border p-4 space-y-3 ${perdida > 0 ? 'border-orange-300' : 'border-slate-200'}`}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Producto {idx + 1}</span>
                {items.length > 1 && (
                  <button type="button" onClick={() => removeItem(item.key)} className="text-slate-400 hover:text-red-500 p-1 transition-colors">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Depósito origen *</label>
                <select
                  value={item.deposito_id}
                  onChange={(e) => updateItem(item.key, 'deposito_id', e.target.value)}
                  className="w-full rounded-xl border border-slate-300 px-4 py-3 text-base text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                >
                  <option value="">Seleccioná…</option>
                  {depositos.map((d) => (
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Retirado {unidad && `(${unidad})`} *</label>
                  <input
                    type="number" inputMode="decimal" min="0" step="0.001"
                    value={item.cantidad_retirada}
                    onChange={(e) => updateItem(item.key, 'cantidad_retirada', e.target.value)}
                    placeholder="0"
                    className="w-full rounded-xl border border-slate-300 px-3 py-3 text-base text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Aplicado {unidad && `(${unidad})`}</label>
                  <input
                    type="number" inputMode="decimal" min="0" step="0.001"
                    value={item.cantidad_aplicada}
                    onChange={(e) => updateItem(item.key, 'cantidad_aplicada', e.target.value)}
                    placeholder="= retirado"
                    className="w-full rounded-xl border border-slate-300 px-3 py-3 text-base text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1.5">Devuelto {unidad && `(${unidad})`}</label>
                  <input
                    type="number" inputMode="decimal" min="0" step="0.001"
                    value={item.cantidad_devuelta}
                    onChange={(e) => updateItem(item.key, 'cantidad_devuelta', e.target.value)}
                    placeholder="0"
                    className="w-full rounded-xl border border-slate-300 px-3 py-3 text-base text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
              ...
              {perdida > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 space-y-2">
                  ...
                  <p className="text-xs text-orange-600">
                    Se imputará como pérdida al costo del lote. Informá la causa:
                  </p>
                  <input
                    type="text"
                    ...
                    className="w-full rounded-xl border border-orange-300 px-3 py-2.5 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-orange-400"
                  />
                </div>
              )}
            </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-green-600 hover:bg-green-700 active:bg-green-800 disabled:opacity-60 text-white font-bold py-4 rounded-2xl text-lg transition-colors shadow-sm"
      >
      ...
      <button
        type="button"
        onClick={() => router.push('/campo')}
        className="w-full bg-white border border-slate-200 text-slate-600 font-semibold py-3.5 rounded-2xl text-base transition-colors"
      >
        Cancelar
      </button>
    </form>
