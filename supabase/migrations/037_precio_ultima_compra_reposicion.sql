-- =============================================================================
-- Migración 037: fn_precio_ultima_compra suma precios de reposición como fallback
-- =============================================================================
-- Cuando un producto no tiene compras ni RIAs con precio, se usa el precio de
-- reposición cargado manualmente (precios_reposicion) como última referencia.
-- Esto evita que productos con precio de mercado conocido (pero nunca comprados
-- en el sistema) queden marcados como "sin precio" en stock, reportes o al
-- importar facturas.
-- =============================================================================
...
LANGUAGE SQL STABLE SECURITY DEFINER AS $$
  SELECT COALESCE(
    ...
    -- 3. Fallback: precio de reposición cargado manualmente (vigente más reciente)
    (
      SELECT pr.precio_ars
      FROM precios_reposicion pr
      WHERE pr.producto_id = p_producto_id
        AND pr.empresa_id  = p_empresa_id
        AND pr.precio_ars  > 0
      ORDER BY pr.vigencia_desde DESC, pr.created_at DESC
      LIMIT 1
    )
  );
$$;
